...
%     K_h = 8e-5*10;
    ...
    m=power(ca, 4.00000)./(power(K_c, 4.00000)+power(ca, 4.00000));
    h_alpha=power(K_h, 4.00000)./(power(K_h, 4.00000)+power(ca, 4.00000));
    plot(ca,m,'LineWidth',2)
    hold on
    plot(ca,h_alpha,'LineWidth',2)
    plot(ca,m.*h_alpha,'LineWidth',2)
    legend({'m','h_{\alpha}','m*h_{\alpha}'})
    xlabel('[Ca^{2+}] (mM)')
    %%
end
if 0
...
function [STATES, CONSTANTS] = initConsts(initStates)
    ...
    global var1;global var3; global var4; global var5;global var6;
    global var7;global var8; global varserca; global varncx
    if isempty(initStates)
        STATES(:,:)=[0.000102316812186319,0.9,0.988494715189347,0.00875264158667971,0.00272874220458962,0.0634814538696518,0.240238948161970];
    else
        STATES(:,:)=initStates;
    end
    CONSTANTS(:,1) = 25.84e3;
    ...
    CONSTANTS(:,29) = 0.35;
    CONSTANTS(:,30) = 0.1;
    CONSTANTS(:,31) = 38.5e-3*varncx;%g_NCX
    CONSTANTS(:,32) = 10;
    CONSTANTS(:,33) = 140;
    CONSTANTS(:,34) = 0.45e-3*varserca;%g_SERCA
    CONSTANTS(:,35) = 0.5e-3;
    ...
    CONSTANTS(:,48) = 1000*var7; %N_IP3R reduced from 20000 to 1000 based on Moschella1993
    CONSTANTS(:,49) = 0.324;
...
function [RATES, ALGEBRAIC] = computeRates(VOI, STATES, CONSTANTS)
    global algebraicVariableCount; global var9;
...
%     RATES(:,7) = ( (ALGEBRAIC(:,60) - STATES(:,7))./CONSTANTS(:,53));%.*(power(CONSTANTS(:,54), 4.00000)+power(STATES(:,1), 4.00000)))./( CONSTANTS(:,53).*power(CONSTANTS(:,54), 4.00000));
    RATES(:,7) = ( (ALGEBRAIC(:,60) - STATES(:,7)).*(power(CONSTANTS(:,54), 4.00000)+power(STATES(:,1), 4.00000)))./( CONSTANTS(:,53).*power(CONSTANTS(:,54), 4.00000));
    ...
    ALGEBRAIC(:,61) = 1.00000e-2*var9;%piecewise({VOI<3300.00, 0.00000 }, 1.00000e-02);
